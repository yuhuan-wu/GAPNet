...
class Block(nn.Module):
    ...
    def __init__(self, dim, out_features, num_heads=4, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, pool_ratios=[12,16,20,24], pooling=True):
        super().__init__()
        ...
        if pooling:
            self.attn = PoolingAttention(
                    ...
                    attn_drop=attn_drop, proj_drop=drop, pool_ratios=pool_ratios, pooling=pooling)
        else:
...
class PyramidPoolingTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],
        ...
        self.block1 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer, pool_ratios=pool_ratios[0], pooling=pooling)
        ...
        self.block2 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer, pool_ratios=pool_ratios[1], pooling=pooling)
        ...
        self.block3 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer, pool_ratios=pool_ratios[2], pooling=pooling)
        ...
        self.block4 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer, pool_ratios=pool_ratios[3], pooling=pooling)
...
def p2t_tiny(pretrained=False, **kwargs):
    model = PyramidPoolingTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 6, 3], pooling=True, **kwargs)
    ...
    if pretrained:
        try:
            ...
            print("weights not available, thus not loaded")
    else:
